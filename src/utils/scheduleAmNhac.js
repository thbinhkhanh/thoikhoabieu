...
export const scheduleAmNhac = ({
  ...
  for (const gv of gvList) {
    if (!gv.monDay.includes("Âm nhạc")) continue;

    const lopList = assignments[gv.id]?.["Âm nhạc"] || [];
    if (!lopList.length) continue;

    const remainingMap = {};
    lopList.forEach(lop => {
      remainingMap[lop] = phanCongGVs[gv.id]?.["Âm nhạc"]?.[lop] || 0;
    });
      ...
      for (const session of allowedSessions) {
        ...
        for (let i = 0; i < sessionArray.length; i++) {
          ...
          const candidates = A.filter(lop =>
            remainingMap[lop] > 0 &&
            countClassInDay(newTkb, gv.id, day, lop, "Âm nhạc") < 1 &&
            !violatesConstraints({ day, session, period: i, mon: "Âm nhạc", lop, gv, tkb: newTkb })
          );
      ...
      for (let i = 0; i < sessionArray.length; i++) {
        ...
        for (const srcSession of ["morning", "afternoon"]) {
          ...
          for (let j = 0; j < srcArray.length; j++) {
            ...
            if (!slot || slot.subject !== "Âm nhạc") continue;
...
export const tryMoveOneSlot_AmNhac = ({
  ...
  if (!slot || slot.subject !== "Âm nhạc") return false;
    ...
    for (const s2 of allowedSessions) {
      ...
      for (let j = 0; j < arr2.length; j++) {
        ...
        if (canPlaceMissingHere && canPlaceOtherThere) {
          arr2[j] = { subject: "Âm nhạc", class: otherLop };
          sessionArray[period] = { subject: "Âm nhạc", class: lopMissing };
      ...
      for (const day of days) {
        for (const session of allowedSessions) {
          ...
          for (let i = 0; i < sessionArray.length; i++) {
            ...
            if (!slot || slot.subject !== "Âm nhạc" || slot.class === lopMissing) continue;
            ...
            // ❌ Không xử lý tiết đôi cho AmNhac
          }
          if (resolved) break;
        }
        if (resolved) break;
...
export const finalFillMissingPeriods_AmNhac = ({
  ...
  for (const gv of gvList) {
    ...
    for (const lopStr of failedLops) {
      ...
      const totalNeeded = assignments[gv.id]?.["Âm nhạc"]?.filter(l => l === lop).length || 0;

      let alreadyAssigned = [...days].reduce((sum, day) => {
        return sum + ["morning", "afternoon"].reduce((s, session) => {
          return s + newTkb[gv.id][day][session].filter(t => t?.subject === "Âm nhạc" && t?.class === lop).length;
        }, 0);
      }, 0);
          ...
          for (let i = 0; i < sessionArray.length; i++) {
            ...
            if (!slot) {
              const countInDay = [...newTkb[gv.id][day].morning, ...newTkb[gv.id][day].afternoon]
                .filter(t => t?.subject === "Âm nhạc" && t?.class === lop).length;
              if (countInDay >= 1) continue;

              if (!violatesConstraints({ day, session, period: i, mon: "Âm nhạc", lop, gv, tkb: newTkb })) {
                assignToSlot(newTkb, globalSchedule, gv.id, lop, "Âm nhạc", { day, session, period: i }, false, assignments, gvList);
            ...
            if (slot?.subject === "Âm nhạc" && slot.class !== lop) {
              const otherLop = slot.class;
                ...
                for (const s2 of allowedSessions) {
                  ...
                  for (let j = 0; j < arr2.length; j++) {
                    ...
                    const countOtherInDay = [...newTkb[gv.id][d2].morning, ...newTkb[gv.id][d2].afternoon]
                      .filter(t => t?.subject === "Âm nhạc" && t?.class === otherLop).length;
                    if (countOtherInDay >= 1) continue;

                    if (!violatesConstraints({ day: d2, session: s2, period: j, mon: "Âm nhạc", lop: otherLop, gv, tkb: newTkb })) {
                      arr2[j] = slot;
                      sessionArray[i] = null;
  ...
  for (const d2 of Object.keys(newTkb[gv.id])) {
    for (const s2 of ["morning", "afternoon"]) {
      ...
      for (let k = 0; k < arr2.length; k++) {
        ...
        if (!current || current.subject !== "Âm nhạc" || current.class === lop) continue;
  ...
  for (const gv of gvList) {
    ...
    for (const day of days) {
      for (const session of ["morning", "afternoon"]) {
        ...
        // ❌ Không gom tiết đôi cho AmNhac

        // ✅ Loại bỏ tiết đầu sáng Thứ 2 nếu có thể
        if (
          day === "Thứ 2" &&
          session === "morning" &&
          sessionArray[0]?.subject === "Âm nhạc"
        ) {
      ...
      for (const day of days) {
        for (const session of ["morning", "afternoon"]) {
          ...
          sessionArray.forEach((t, i) => {
            if (t?.subject === "Âm nhạc" && t?.class === lop) {
  ...
  for (const gv of gvList) {
    ...
    for (const lopStr of failedLops) {
      ...
      for (const day of days) {
        ...
        const morningSlots = morningArr.filter(t => t?.subject === "Âm nhạc" && t?.class === lop);
        const afternoonSlots = afternoonArr.filter(t => t?.subject === "Âm nhạc" && t?.class === lop);
        ...
        for (const session of ["morning", "afternoon"]) {
          ...
          for (let i = 0; i < sessionArray.length; i++) {
            ...
            // 🔁 Nếu slot đã có lớp khác → thử hoán đổi
            else if (slot.subject === "Âm nhạc" && slot.class !== lop) {
              const otherLop = slot.class;
...
export const optimizeFullAmNhacSchedule = ({
  ...
  // ❌ Không gọi splitDoublePeriodsIfNeeded vì AmNhac không có tiết đôi
