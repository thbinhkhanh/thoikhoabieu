...
export const scheduleDocSach = ({
  ...
  for (const gv of gvList) {
    if (!gv.monDay.includes("Đọc sách")) continue;

    const lopList = assignments[gv.id]?.["Đọc sách"] || [];
    if (!lopList.length) continue;

    const remainingMap = {};
    lopList.forEach(lop => {
      remainingMap[lop] = phanCongGVs[gv.id]?.["Đọc sách"]?.[lop] || 0;
    });
      ...
      for (const session of allowedSessions) {
        ...
        for (let i = 0; i < sessionArray.length; i++) {
          ...
          const candidates = A.filter(lop =>
            remainingMap[lop] > 0 &&
            countClassInDay(newTkb, gv.id, day, lop, "Đọc sách") < 1 &&
            !violatesConstraints({ day, session, period: i, mon: "Đọc sách", lop, gv, tkb: newTkb })
          );
 ...
 */
function restructureDocSachSchedule({
  newTkb,
  gv,
  remainingMap,
  lopList,
      ...
      for (let i = 0; i < sessionArray.length; i++) {
        ...
        for (const srcSession of ["afternoon"]) {
          ...
          for (let j = 0; j < srcArray.length; j++) {
            ...
            if (!slot || slot.subject !== "Đọc sách") continue;
 ...
 */
export const countClassInDayDocSach = (
  tkb,
  gvId,
  day,
  className,
...
export const tryPlaceSingleAt = ({
  ...
  if (totalAssignedToday >= 2) return false; // GV đã có 1 tiết → không xếp thêm
  ...
  const countInDay = countClassInDayDocSach(tkb, gvId, day, lop, subject);
 ...
 */
export const fillSessionForGVDocSach = ({
  gv,
  lopList,
  remainingMap,
  tkb,
    ...
    for (const session of allowedSessions) {
      ...
      for (let i = 0; i < sessionArray.length; i++) {
        ...
        const candidates = shuffledLopList
          .filter(lop => (remainingMap[lop] || 0) > 0)
          .filter(lop => countClassInDayDocSach(tkb, gvId, day, lop) < 1)
 ...
 */
export const tryMoveOneSlot_DocSach = ({
  tkb,
  globalSchedule,
  gvId,
  day,
  ...
  if (!slot || slot.subject !== "Đọc sách") return false;
    ...
    for (const s2 of allowedSessions) {
      ...
      for (let j = 0; j < arr2.length; j++) {
        ...
        const canPlaceMissingHere =
          ...
          countClassInDayDocSach(tkb, gvId, day, lopMissing) < 1;

        const countOtherInD2 = countClassInDayDocSach(tkb, gvId, d2, otherLop);
        ...
        if (canPlaceMissingHere && canPlaceOtherThere) {
          arr2[j] = { subject: "Đọc sách", class: otherLop };
          sessionArray[period] = { subject: "Đọc sách", class: lopMissing };
      ...
      for (const day of days) {
        for (const session of allowedSessions) {
          ...
          for (let i = 0; i < sessionArray.length; i++) {
            ...
            if (!slot || slot.subject !== "Đọc sách" || slot.class === lopMissing) continue;

            const moved = tryMoveOneSlot_DocSach({
              tkb,
              globalSchedule,
              gvId,
              day,
            ...
            // ❌ Không xử lý tiết đôi cho DocSach
          }
          if (resolved) break;
        }
        if (resolved) break;
 ...
 */
export const finalFillMissingPeriods_DocSach = ({
  gvList = [],
  newTkb,
  globalSchedule,
  assignments,
  ...
  for (const gv of gvList) {
    ...
    for (const lopStr of failedLops) {
      ...
      const totalNeeded = assignments[gv.id]?.["Đọc sách"]?.filter(l => l === lop).length || 0;

      let alreadyAssigned = [...days].reduce((sum, day) => {
        return sum + ["morning", "afternoon"].reduce((s, session) => {
          return s + newTkb[gv.id][day][session].filter(t => t?.subject === "Đọc sách" && t?.class === lop).length;
        }, 0);
      }, 0);
          ...
          for (let i = 0; i < sessionArray.length; i++) {
            ...
            if (!slot) {
              const countInDay = [...newTkb[gv.id][day].morning, ...newTkb[gv.id][day].afternoon]
                .filter(t => t?.subject === "Đọc sách" && t?.class === lop).length;
              if (countInDay >= 1) continue;

              if (!violatesConstraints({ day, session, period: i, mon: "Đọc sách", lop, gv, tkb: newTkb })) {
                assignToSlot(newTkb, globalSchedule, gv.id, lop, "Đọc sách", { day, session, period: i }, false, assignments, gvList);
            ...
            if (slot?.subject === "Đọc sách" && slot.class !== lop) {
              const otherLop = slot.class;
                ...
                for (const s2 of allowedSessions) {
                  ...
                  for (let j = 0; j < arr2.length; j++) {
                    ...
                    const countOtherInDay = [...newTkb[gv.id][d2].morning, ...newTkb[gv.id][d2].afternoon]
                      .filter(t => t?.subject === "Đọc sách" && t?.class === otherLop).length;
                    if (countOtherInDay >= 1) continue;

                    if (!violatesConstraints({ day: d2, session: s2, period: j, mon: "Đọc sách", lop: otherLop, gv, tkb: newTkb })) {
                      arr2[j] = slot;
                      sessionArray[i] = null;
  ...
  for (const d2 of Object.keys(newTkb[gv.id])) {
    ...
    for (const s2 of ["afternoon"]) {
      ...
      for (let k = 0; k < arr2.length; k++) {
        ...
        if (!current || current.subject !== "Đọc sách" || current.class === lop) continue;
 ...
 */
export const checkTietPerDay_DocSach = ({ newTkb, gvList, days }) => {
  const violations = [];
 ...
 */
{/*export const optimizeDocSachSchedule = ({
  newTkb,
  gvList,
  assignments,
  days
  ...
  for (const gv of gvList) {
    ...
    for (const day of days) {
      for (const session of ["morning", "afternoon"]) {
        ...
        // ❌ Không gom tiết đôi cho DocSach

        // ✅ Loại bỏ tiết đầu sáng Thứ 2 nếu có thể
        if (
          day === "Thứ 2" &&
          session === "morning" &&
          sessionArray[0]?.subject === "Đọc sách"
        ) {
  ...
  const violations = checkTietPerDay_DocSach({ newTkb, gvList, days });
 ...
 */
{/*export const groupDocSachByClass = ({ newTkb, gvList, assignments, days }) => {
  for (const gv of gvList) {
    if (!gv.monDay.includes("Đọc sách")) continue;
      ...
      for (const day of days) {
        for (const session of ["morning", "afternoon"]) {
          ...
          sessionArray.forEach((t, i) => {
            if (t?.subject === "Đọc sách" && t?.class === lop) {
 ...
 */
{/*export const regroupDocSachSlots = ({ newTkb, gvList, assignments, days }) => {
  for (const gv of gvList) {
    if (!gv.monDay.includes("Đọc sách")) continue;
 ...
 */
export const resolveMissingDocSach = ({
  newTkb,
  gvList,
  gvSummary,
  assignments,
  ...
  for (const gv of gvList) {
    ...
    for (const lopStr of failedLops) {
      ...
      for (const day of days) {
        ...
        const morningSlots = morningArr.filter(t => t?.subject === "Đọc sách" && t?.class === lop);
        const afternoonSlots = afternoonArr.filter(t => t?.subject === "Đọc sách" && t?.class === lop);
        ...
        for (const session of ["morning", "afternoon"]) {
          ...
          for (let i = 0; i < sessionArray.length; i++) {
            ...
            // 🔁 Nếu slot đã có lớp khác → thử hoán đổi
            else if (slot.subject === "Đọc sách" && slot.class !== lop) {
              const otherLop = slot.class;
 ...
 */
export const optimizeFullDocSachSchedule = ({
  newTkb,
  gvList,
  assignments,
  days,
  ...
  //optimizeDocSachSchedule({ newTkb, gvList, assignments, days });
  //groupDocSachByClass({ newTkb, gvList, assignments, days });
  //regroupDocSachSlots({ newTkb, gvList, assignments, days });
  // ❌ Không gọi splitDoublePeriodsIfNeeded vì DocSach không có tiết đôi
  resolveMissingDocSach({ newTkb, gvList, gvSummary, assignments, globalSchedule, days });
};
